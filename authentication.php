...
if (isset($_POST['btnsubmit'])) {
    ...
    if ($_POST['btnsubmit'] == "Login") {
        ...
        if (empty($username) || empty($password)) {
            ...
            if(count($userData) > 0) {
                ...
                if($row == md5($password)){
                        $_SESSION['user_id'] = base64_encode($userData[0]['id']);
    ...
    /* Upload User Image */
    else if($_POST['btnsubmit'] == "Upload User Image"){
        $user_id = base64_decode($_SESSION['user_id']);
        $checkUser = getUserData($conn,$user_id,"","");
        $file_path = "images_user/"; //Folder where the image will be saved
                    ...
                    if($_FILES["user_image"]["size"] < 5000000){
                        ...
                        if(move_uploaded_file($_FILES['user_image']['tmp_name'],$save_file)){
                            ...
                            if(mysqli_query($conn,$updateImageQuery)){
                                ...
                                //echo "Failed to save user image, Try Again!";
                                $_SESSION['error'] = "0";
                                $_SESSION['message'] = "Failed to save user image, Try Again!";
                                header("Location:profile.php");
                            }
                        }
                        else{
